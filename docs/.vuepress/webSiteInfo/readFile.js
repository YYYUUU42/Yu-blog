...
import matter from 'gray-matter'; // FrontMatter解析器
import chalk from 'chalk'; // 命令行打印美化

const log = console.log;
const docsRoot = path.join(process.cwd(), 'docs'); // 使用 process.cwd() 获取当前工作目录
...
function readTime(len, cn = 300, en = 160) {
    ...
    if (readingTime > 60 && readingTime < 60 * 24) {   // 大于一个小时，小于一天
        ...
        const minute = parseInt((readingTime - hour * 60));
        if (minute === 0) {
            return hour + 'h';
        }
        return hour + 'h' + minute + 'm';
    } else if (readingTime > 60 * 24) {      // 大于一天
        const day = parseInt(readingTime / (60 * 24));
        const hour = parseInt((readingTime - day * 24 * 60) / 60);
        if (hour === 0) {
            return day + 'd';
        }
        return day + 'd' + hour + 'h';
